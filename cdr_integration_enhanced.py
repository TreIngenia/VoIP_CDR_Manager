#!/usr/bin/env python3
"""
CDR Integration Enhanced - Sistema integrato per elaborazione CDR con categorie configurabili
Sostituisce i sistemi precedenti con un approccio unificato basato su cdr_categories.py
"""
...
# Import del sistema categorie
from cdr_categories import CDRCategoriesManager

logger = logging.getLogger(__name__)

class CDRAnalyticsIntegrated:
    """
    ...
    def _enhance_records_with_categories(self, records: List[Dict]) -> List[Dict]:
        ...
        for record in records:
            try:
                ...
                # In caso di errore, mantieni record originale con valori di fallback
    ...
    def _generate_contract_report(self, contract_code: str, records: List[Dict], metadata: Dict) -> Optional[str]:
        ...
        try:
            ...
            # ✅ INFORMAZIONI CATEGORIE CONFIGURAZIONE
    ...
    def _get_top_categories_by_cost(self, costo_by_category: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        top_categories = []
        for cat_name, cost in sorted_categories[:5]:  # Top 5
    ...
    def _generate_summary_report(self, grouped_data: Dict, metadata: Dict) -> Optional[str]:
        ...
        try:
            ...
            # ✅ AGGREGAZIONE GLOBALE PER CATEGORIA (CAMPO RICHIESTO)
            ...
            for contract_code, records in grouped_data.items():
                ...
                # ✅ AGGREGA COSTI PER CATEGORIA A LIVELLO GLOBALE
            ...
            # ✅ FINALIZZA CAMPO RICHIESTO PRINCIPALE
            ...
            for cat_name, data in global_category_stats.items():
                ...
                data['avg_cost_per_call'] = round(data['cost_client_euro'] / data['calls'], 4) if data['calls'] > 0 else 0
                data['match_rate'] = round((data['matched_calls'] / data['calls']) * 100, 1) if data['calls'] > 0 else 0
                # Converte set in lista per JSON
                data['contracts_using'] = list(data['contracts_using'])
            
            # ✅ REPORT SUMMARY FINALE CON CATEGORIE
            summary_report = {
                'metadata': {
                    'generation_timestamp': now.isoformat(),
                    'source_file': metadata.get('source_file', ''),
                ...
                # ✅ CAMPO RICHIESTO PRINCIPALE A LIVELLO GLOBALE
                ...
                'category_performance_analysis': self._analyze_category_performance_enhanced(global_category_stats),
                'system_health': {
                    'matched_calls_percentage': self._calculate_global_match_rate(global_category_stats, global_totals['total_calls']),
                    'categories_coverage': self._analyze_categories_coverage(global_category_stats),
                    'recommendations': self._generate_recommendations(global_category_stats)
                }
            }
    ...
    def _get_top_categories_global(self, global_costs: Dict[str, float]) -> List[Dict]:
        ...
        for cat_name, cost in sorted_categories:
            top_categories.append({
                ...
                'percentage_of_total': 0  # Verrà calcolato successivamente se necessario
            })
    ...
    def _analyze_category_performance_enhanced(self, global_stats: Dict) -> Dict[str, Any]:
        """Analisi performance categorie avanzata"""
        try:
            performance = {}
            
            for cat_name, data in global_stats.items():
                efficiency_score = 0
                if data['calls'] > 0 and data['duration_seconds'] > 0:
                    # Score basato su: utilizzo, copertura, match rate
                    usage_score = min(data['calls'] / 100, 1.0)  # Normalizzato su 100 chiamate
                    coverage_score = min(data['contracts_count'] / 10, 1.0)  # Normalizzato su 10 contratti
                    match_score = data['match_rate'] / 100  # Già in percentuale
                    
                    efficiency_score = round((usage_score + coverage_score + match_score) / 3, 3)
                
                performance[cat_name] = {
                    'display_name': data['display_name'],
                    'calls_count': data['calls'],
                    'contracts_coverage': data['contracts_count'],
                    'avg_cost_per_call': data['avg_cost_per_call'],
                    'total_revenue_potential': data['cost_client_euro'],
                    'efficiency_score': efficiency_score,
                    'match_rate': data['match_rate'],
                    'market_share_percentage': data['percentage_of_total'],
                    'price_per_minute': data['price_per_minute'],
                    'recommendation': self._get_category_recommendation(data, efficiency_score)
                }
            
            # Ordina per efficiency score
            performance = dict(sorted(performance.items(), key=lambda x: x[1]['efficiency_score'], reverse=True))
    ...
    def _get_category_recommendation(self, stats: Dict, efficiency_score: float) -> str:
        """Genera raccomandazione per una categoria"""
        if stats['match_rate'] < 50:
            return "Migliorare pattern di riconoscimento"
        elif efficiency_score > 0.8:
            return "Performance ottimale"
        elif stats['calls'] < 10:
            return "Categoria poco utilizzata"
        elif stats['contracts_count'] < 2:
            return "Estendere copertura contratti"
        else:
            return "Monitorare performance"
    
    def _calculate_global_match_rate(self, global_stats: Dict, total_calls: int) -> float:
        """Calcola tasso di riconoscimento globale"""
        total_matched = sum(stats['matched_calls'] for stats in global_stats.values())
        return round((total_matched / total_calls) * 100, 2) if total_calls > 0 else 0
    
    def _analyze_categories_coverage(self, global_stats: Dict) -> Dict[str, Any]:
        """Analizza copertura delle categorie"""
        active_categories = len([s for s in global_stats.values() if s['calls'] > 0])
        total_configured = len(self.categories_manager.get_all_categories())
        
        return {
            'active_categories': active_categories,
            'total_configured': total_configured,
            'coverage_percentage': round((active_categories / total_configured) * 100, 2) if total_configured > 0 else 0,
            'unused_categories': total_configured - active_categories
        }
    
    def _generate_recommendations(self, global_stats: Dict) -> List[str]:
        """Genera raccomandazioni sistema"""
        recommendations = []
        
        # Controlla match rate globale
        total_calls = sum(stats['calls'] for stats in global_stats.values())
        total_matched = sum(stats['matched_calls'] for stats in global_stats.values())
        global_match_rate = (total_matched / total_calls * 100) if total_calls > 0 else 0
        
        if global_match_rate < 85:
            recommendations.append(f"Tasso riconoscimento globale basso ({global_match_rate:.1f}%). Rivedere pattern categorie.")
        
        # Controlla categorie poco utilizzate
        low_usage_categories = [name for name, stats in global_stats.items() if stats['calls'] < 5]
        if low_usage_categories:
            recommendations.append(f"Categorie poco utilizzate: {', '.join(low_usage_categories[:3])}{'...' if len(low_usage_categories) > 3 else ''}")
        
        # Controlla distribuzione costi
        costs = [stats['cost_client_euro'] for stats in global_stats.values()]
        if costs and max(costs) > sum(costs) * 0.7:  # Una categoria domina
            dominant_cat = max(global_stats.items(), key=lambda x: x[1]['cost_client_euro'])
            recommendations.append(f"Categoria {dominant_cat[0]} domina i costi. Verificare bilanciamento prezzi.")
        
        if not recommendations:
            recommendations.append("Sistema categorie funziona correttamente.")
...
def process_cdr_with_categories_standalone(json_file_path: str, output_directory: str = "output") -> Dict:
    ...
    """
    analytics = CDRAnalyticsIntegrated(output_directory)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Uso: python cdr_integration_enhanced.py <file_cdr.json>")
        print("Esempio: python cdr_integration_enhanced.py output/RIV_12345_MESE_05_2024-06-05-14.16.27.json")